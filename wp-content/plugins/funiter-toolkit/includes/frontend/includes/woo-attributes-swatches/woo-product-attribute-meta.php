<?php
if ( ! class_exists( 'Funiter_Attribute_Product_Meta' ) ) {
	class Funiter_Attribute_Product_Meta {
		public $plugin_uri;
		public $screen;
		public $taxonomy;
		...
		public function scripts() {
			if ( class_exists( 'WooCommerce' ) ) {
				wp_register_style( 'funiter-toolkit-woo-attribute', $this->plugin_uri . 'woo-attribute.css', array(), '1.0' );
				wp_register_script( 'funiter-toolkit-woo-attribute', $this->plugin_uri . 'woo-attribute.js', array(), '1.0', true );
				
				$product_thumb_id = 0;
				if ( is_singular( 'product' ) ) {
					$product_thumb_id = get_post_thumbnail_id( get_the_ID() );
				}
				$default_thumb = funiter_toolkit_resize_image( $product_thumb_id, null, 100, 100, true, true, false );
				wp_localize_script( 'funiter-toolkit-woo-attribute', 'funiter_toolkit_woo_atts', array(
					'text'           => array(
						'select_variation' => esc_html__( 'Select variation', 'funiter-toolkit' )
					),
		...
		public function product_attributes_type_selector( $types ) {
			$funiter_types = array(
				'box_style' => esc_html__( 'Box Style', 'funiter' ),
			);
			
			return array_merge( $types, $funiter_types );
		}
		...
		public function add_attribute_fields() {
            ...
            <div class="form-field" style="display: none;">
                <label><?php esc_html_e( 'Size Box', 'funiter' ); ?></label>
                <input type="text" name="attribute_size" id="attribute_size" value="30x30">
                <p class="description"><?php esc_html_e( 'Determines Size in this attribute.{width}x{height}', 'funiter' ); ?></p>
            </div>
		...
		public function edit_attribute_fields() {
            ...
            <tr class="form-field form-required" <?php if ( $attribute->attribute_type == 'select' ): ?> style="display: none;" <?php endif; ?>>
                <th scope="row" valign="top"><label><?php _e( 'Size Box', 'funiter' ); ?></label></th>
                <td>
		...
		public function product_option_terms( $attribute_taxonomy, $i ) {
			...
			if ( 'box_style' === $attribute_taxonomy->attribute_type ) : ?>
                <select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'funiter' ); ?>"
                ...
                <button class="button plus select_all_attributes"><?php esc_html_e( 'Select all', 'funiter' ); ?></button>
                <button class="button minus select_no_attributes"><?php esc_html_e( 'Select none', 'funiter' ); ?></button>
                <button class="button fr plus add_new_attribute"><?php esc_html_e( 'Add new', 'funiter' ); ?></button>
				<?php
			endif;
		}
		...
		public function add_attr_field() {
            ...
            <div class="form-field term-display-type-wrap">
                <label for="display_type"><?php _e( 'Display type', 'funiter' ); ?></label>
                <select name="<?php echo esc_attr( $id_type ); ?>" id="product_attribute_type_id">
                    <option value="-1"><?php echo esc_html__( 'None', 'funiter' ); ?></option>
                    <option value="color"><?php echo esc_html__( 'Color', 'funiter' ); ?></option>
                    <option value="photo" selected><?php echo esc_html__( 'Photo', 'funiter' ); ?></option>
                    <option value="label"><?php echo esc_html__( 'Label', 'funiter' ); ?></option>
                </select>
                <script type="text/javascript">
            ...
            <div class="form-field term-display-type-wrap swatch photo">
                <label><?php _e( 'Thumbnail', 'funiter' ); ?></label>
                ...
                <div style="line-height: 60px;">
                    ...
                    <button type="button"
                            class="upload_image_button button"><?php _e( 'Upload/Add image', 'funiter' ); ?></button>
                    <button type="button"
                            class="remove_image_button button"><?php _e( 'Remove image', 'funiter' ); ?></button>
                </div>
                <script type="text/javascript">
                    ...
                    jQuery(document).on('click', '.upload_image_button', function (event) {
                        ...
                        file_frame = wp.media.frames.downloadable_file = wp.media({
                            title: '<?php _e( 'Choose an image', 'funiter' ); ?>',
                            button: {
                                text: '<?php _e( 'Use image', 'funiter' ); ?>'
                            },
                            multiple: false
                        });
		...
		public function edit_attr_field( $term, $taxonomy ) {
			...
			$id_color    = "product_attribute_meta[{$this->meta_key}][color]";
			$id_photo    = "product_attribute_meta[{$this->meta_key}][photo]";
			$swatch_term = new Funiter_Term( $this->meta_key, $term->term_id, $taxonomy, false );
            ...
            <tr class="form-field swatch color <?php echo $swatch_term->get_type() == 'color' ? 'active' : ''; ?>">
                <th scope="row" valign="top"><label><?php _e( '', 'funiter' ); ?></label></th>
                <td>
		...
		public function product_attr_columns( $columns ) {
			...
			$new_columns['thumb'] = __( 'Image', 'funiter' );
		...
		public function product_attr_column( $columns, $column, $id ) {
			if ( 'thumb' === $column ) {
				$swatch_term = new Funiter_Term( $this->meta_key, $id, $this->taxonomy, false );
		...
		public function wc_variation_attribute_options( $html, $args ) {
			...
			$args                  = wp_parse_args( apply_filters( 'woocommerce_dropdown_variation_attribute_options_args', $args ), array(
				                                                                                                                       ...
				                                                                                                                       'id'               => '',
				                                                                                                                       'class'            => '',
				                                                                                                                       'show_option_none' => __( 'Choose an option', 'funiter' ),
			                                                                                                                       )
			);
			...
			$show_option_none_text = $args['show_option_none'] ? $args['show_option_none'] : __( 'Choose an option', 'funiter' ); // We'll do our best to hide the placeholder, but we'll need to show something when resetting options.
			...
			if ( ! empty( $options ) ) {
				if ( $product && taxonomy_exists( $attribute ) ) {
					...
					wp_enqueue_style( 'funiter-toolkit-woo-attribute' );
					wp_enqueue_script( 'funiter-toolkit-woo-attribute' );
		...
		public function get_product_attribute( $attribute ) {
			...
			try {
				...
				if ( is_wp_error( $attribute ) || is_null( $attribute ) ) {
					throw new WC_API_Exception( 'woocommerce_api_invalid_product_attribute_id', __( 'A product attribute with the provided ID could not be found', 'funiter' ), 404 );
				}
	...
	new Funiter_Attribute_Product_Meta();
}