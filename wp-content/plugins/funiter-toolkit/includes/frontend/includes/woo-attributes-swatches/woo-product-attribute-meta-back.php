...
if ( ! class_exists( 'Funiter_Attribute_Product_Meta' ) ) {
	class Funiter_Attribute_Product_Meta {
		...
		public function __construct( $attribute_image_key = 'attribute_swatch', $image_size = 'shop_thumb' ) {
			...
			if ( is_admin() ) {
				...
				add_action( 'woocommerce_product_option_terms', array(
					$this,
					'funiter_woocommerce_product_option_terms'
				), 10, 3 );
			}
		...
		public function funiter_woocommerce_product_option_terms( $attribute_taxonomy, $i ) {
		...
		public function woocommerce_add_attribute_thumbnail_field() {
            ...
            <div class="form-field ">
                <label for="product_attribute_type_<?php echo $this->meta_key; ?>"><?php esc_html_e( 'Type', 'funiter' ) ?></label>
                <select name="product_attribute_meta[<?php echo $this->meta_key; ?>][type]"
                    ...
                    <option value="-1"><?php esc_html_e( 'None', 'funiter' ) ?></option>
                    <option value="color"><?php esc_html_e( 'Color', 'funiter' ) ?></option>
                    <option value="photo"><?php esc_html_e( 'Photo', 'funiter' ) ?></option>
                    <option value="label"><?php esc_html_e( 'Label', 'funiter' ) ?></option>
                </select>
            ...
            <div class="form-field swatch-field field-color section-color-swatch"
                ...
                <div id="swatch-color" class="<?php echo sanitize_title( $this->meta_key ); ?>-color">
                    <label><?php esc_html_e( 'Color', 'funiter' ); ?></label>
            ...
            <div class="form-field swatch-field field-photo" style="overflow:visible;display:none;">
                <div id="swatch-photo" class="<?php echo sanitize_title( $this->meta_key ); ?>-photo">
                    <label><?php esc_html_e( 'Thumbnail', 'funiter' ); ?></label>
                    ...
                    <div style="line-height:60px;">
                        ...
                        <button type="submit"
                                class="upload_image_button button"><?php esc_html_e( 'Upload/Add image', 'funiter' ); ?></button>
                        <button type="submit"
                                class="remove_image_button button"><?php esc_html_e( 'Remove image', 'funiter' ); ?></button>
                    </div>
                    <script type="text/javascript">
		...
		public function woocommerce_edit_attributre_thumbnail_field( $term, $taxonomy ) {
			global $woocommerce;
			$swatch_term = new Funiter_Term( $this->meta_key, $term->term_id, $taxonomy, false, $this->image_size );
			$image       = '';
			?>
            <tr class="form-field ">
                <th scope="row" valign="top"><label><?php esc_html_e( 'Type', 'funiter' ); ?></label></th>
                <td>
                    <label>
                        <select name="product_attribute_meta[<?php echo $this->meta_key; ?>][type]"
                                id="product_attribute_swatchtype_<?php echo $this->meta_key; ?>" class="postform">
                            <option <?php selected( 'none', $swatch_term->get_type() ); ?>
                                    value="-1"><?php esc_html_e( 'None', 'funiter' ); ?></option>
                            <option <?php selected( 'color', $swatch_term->get_type() ); ?>
                                    value="color"><?php esc_html_e( 'Color', 'funiter' ); ?></option>
                            <option <?php selected( 'photo', $swatch_term->get_type() ); ?>
                                    value="photo"><?php esc_html_e( 'Photo', 'funiter' ); ?></option>
                            <option <?php selected( 'label', $swatch_term->get_type() ); ?>
                                    value="label"><?php esc_html_e( 'Label', 'funiter' ); ?></option>
                        </select>
                    </label>
                    <script type="text/javascript">
                        jQuery(document).ready(function ($) {
            ...
            <tr class="form-field swatch-field swatch-field-color section-color-swatch"
                ...
                <th scope="row" valign="top"><label><?php esc_html_e( 'Color', 'funiter' ); ?></label></th>
                <td>
                    <div id="swatch-color" class="<?php echo sanitize_title( $this->meta_key ); ?>-color">
                        <div id="product_attribute_color_<?php echo $this->meta_key; ?>_picker" class="colorSelector">
                            <div></div>
            ...
            <tr class="form-field swatch-field swatch-field-photo" style="overflow:visible;<?php echo $style; ?>">
                <th scope="row" valign="top"><label><?php esc_html_e( 'Photo', 'funiter' ); ?></label></th>
                <td>
                    <div id="product_attribute_thumbnail_<?php echo $this->meta_key; ?>"
                         style="float:left;margin-right:10px;">
		...
		public function woocommerce_product_attribute_columns( $columns ) {
			...
			$new_columns[ $this->meta_key ] = esc_html__( 'Thumbnail', 'funiter' );
			unset( $columns['cb'] );
		...
		public function woocommerce_product_attribute_column( $columns, $column, $id ) {
			if ( $column == $this->meta_key ) :
				$swatch_term = new Funiter_Term( $this->meta_key, $id, $this->taxonomy, false, $this->image_size );
				...
				if ( $product && taxonomy_exists( $attribute ) ) {
					...
					foreach ( $terms as $term ) {
						if ( in_array( $term->slug, $options ) ) {
							...
							if ( has_filter( 'funiter_resize_image' ) ) {
								// $image_thumb = funiter_resize_image( $data_photo, null, $attribute_swatch_width, $attribute_swatch_height, true, true, false );
								$image_thumb = apply_filters( 'funiter_resize_image', $data_photo, $attribute_swatch_width, $attribute_swatch_height, true, true );
