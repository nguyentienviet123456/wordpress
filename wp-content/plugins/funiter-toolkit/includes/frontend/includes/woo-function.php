 ...
 */
add_action( 'woocommerce_before_single_product_summary', 'funiter_show_product_extent', 10 );
if ( !function_exists( 'funiter_show_product_extent' ) ) {
	function funiter_show_product_extent()
	{
		global $product;
		...
		if ( isset( $product_meta['product_options'] ) && $product_meta['product_options'] == 'video' && isset( $product_meta['video_product_url'] ) && $product_meta['video_product_url'] != '' ) {
			echo '<div class="product-video-button"><a href="' . esc_url( $product_meta['video_product_url'] ) . '">' . esc_html__( 'Watch Video', 'funiter' ) . '</a></div>';
		}
		if ( isset( $product_meta['product_options'] ) && $product_meta['product_options'] == '360deg' && isset( $product_meta['degree_product_gallery'] ) && $product_meta['degree_product_gallery'] != '' ) : ?>
			...
			<div id="product-360-view" class="product-360-view-wrapper mfp-hide">
				<div class="funiter-threed-view threed-id-<?php echo esc_attr( $id ); ?>">
					<?php if ( !empty( $title ) ): ?>
			...
			<div class="product-360-button">
				<a href="#product-360-view"><span><?php echo esc_html__( '360 Degree', 'funiter' ); ?></span></a>
			</div>
		<?php
		endif;
	}
