jQuery(function ($) {
    ...
    var initPopup = function () {
        var pin = $('.funiter-pin .action-pin');

        pin.on('click', function () {

            var _this = $(this),
                popup = _this.siblings('.funiter-popup');

            if ( !popup.length ) {
                return;
            }

            var parent = _this.closest('.funiter-pin');
            ...
            $('.funiter-mapper .funiter-pin .funiter-popup-header h2').css('max-width', popup_width - 10);

            // Set height content for image type
            if ( popup.hasClass('funiter-image') ) {
                var popup_header_height = popup.find('.funiter-popup-header').outerHeight(true);
                popup.find('.funiter-popup-main').css('height', (popup_height - popup_header_height));
            }

            // Add Actived class
            setTimeout(function () {
                // Remove all pin actived class
                $('.funiter-mapper .funiter-pin.actived').removeClass('actived');
        ...
        $('.funiter-pin .close-modal').on('click', function () {
            var parent = $(this).closest('.funiter-pin'),
                popup  = parent.find('.funiter-popup');
        ...
        pin.hover(function () {
            ...
            flag = false;
            _this.closest('.funiter-mapper').children('img').removeAttr('style');
