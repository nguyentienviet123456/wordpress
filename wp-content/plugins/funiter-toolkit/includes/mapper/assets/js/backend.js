jQuery(function ($) {
    ...
    var initTab = function () {
        $(document).on('click', '.funiter-mapper ul.nav li', function (e) {
    ...
    var imageSelector = function () {
        $(document).on('click', '.funiter-mapper a.btn-add-image, .funiter-mapper #change-image, .funiter-mapper a.image-selector', function (event) {
            ...
            if (!window.wr_image_selector) {
                ...
                window.wr_image_selector = wp.media({
                    button: {
                        text: funiter_mapper.text.img_selector_btn_label,
                    },
                    states: [new wp.media.controller.Library({
                        title: funiter_mapper.text.img_selector_modal_title,
                ...
                window.wr_image_selector.on('select', function () {
                    ...
                    if (window.wr_image_selector.input_element.attr('id') == 'funiter_mapper_image') {
                        window.wr_image_selector.input_element.val(attachment.attributes.id);

                        // Update edit area with new image.
                        if ($('.funiter-mapper-bot > .edit-image').length) {
                            $('.funiter-mapper-bot .image-wrap > img').attr('src', attachment.attributes.url);
                        } else {
                            var edit_html = $('#funiter_mapper_image_tmpl').text().replace('%URL%', attachment.attributes.url);

                            $('.funiter-mapper-bot > .add-image').replaceWith(edit_html);
            ...
            window.wr_image_selector.input_element = $(this).hasClass('image-selector') ? $(this).prev('input') : $('#funiter_mapper_image');
    ...
    var setupApplication = function () {
        ...
        var PinList = Backbone.Collection.extend({
            ...
            fetch: function () {
                if (window.funiter_mapper_pins && window.funiter_mapper_pins.length) {
                    this.add(window.funiter_mapper_pins);
                }
            },
        });
        ...
        var PinView = Backbone.View.extend({
            tagName: 'div',
            className: 'funiter-pin',

            // Cache the template function for a single item.
            template: _.template($('#funiter_mapper_pin_tmpl').text()),
            ...
            render: function () {
                ...
                this.$el.find('[data-option]').each(function () {
                    ...
                    if ($(this).attr('name') != option[0]) {
                        if (option[3] !== undefined) {
                            $(this).attr('name', 'funiter_mapper_pins[' + self.index + '][' + option[1] + '][' + option[3] + ']');
                        } else {
                            $(this).attr('name', 'funiter_mapper_pins[' + self.index + '][' + option[1] + ']');
                        }
                    }
                    ...
                    if ($(this).hasClass('color-picker')) {
                        ...
                        setTimeout($.proxy(function () {
                            $(this).parent().parent().click(function () {
                                ...
                                // focused.
                                $('.funiter-mapper .icon-selector').each(function () {
                    ...
                    // Init icon selector if needed.
                        ...
                        $(this).addClass('hidden').after($('#funiter_mapper_icon_selector_tmpl').text().replace('%SELECTED%', $(this).val()));
                    }
                    ...
                    // Init product selector if needed.
                        ...
                        $(this).select2({
                            ...
                            ajax: {
                                url: funiter_mapper.product_selector.url,
                                dataType: 'json',
                                delay: 250,
                                data: function (terms) {
                                    return {
                                        term: terms,
                                        security: funiter_mapper.product_selector.security,
                                    };
                                },
                            ...
                            initSelection: function (element, callback) {
                                ...
                                if (id !== '') {
                                    $.ajax(funiter_mapper.product_selector.url + '&term=' + id + '&security=' + funiter_mapper.product_selector.security, {
                                        dataType: 'json',
                                    }).done(function (data) {
                                        callback(data);
                                    });
                ...
                }).on('click', '.funiter-icon-list a', function (event) {
                    event.preventDefault();
            ...
            edit: function (event) {
                if (!$(event.target).parent().hasClass('funiter-pin') || this.just_dragged) {
                    return;
                }
                ...
                if (this.$el.hasClass('editing')) {
                    ...
                    if (!this.$el.data('funiter_mapper_pin_settings_initialized')) {
                        var self = this;
                        ...
                        this.$el.data('funiter_mapper_pin_settings_initialized', true);
                    }
            ...
            close: function (event) {
                ...
                if (pin_type == 'woocommerce') {
                    ...
                    this.$el.find('.tooltip').text(title ? title : funiter_mapper.text.please_input_a_title).removeClass('hidden');
                }
            ...
            remove: function (event) {
                ...
                if (confirm(funiter_mapper.text.confirm_removing_pin)) {
                    this.$el.remove();
                    this.model.destroy();
        ...
        var PinListView = Backbone.View.extend({
            ...
            el: '.funiter-mapper-bot > .edit-image > .image-wrap',
            ...
            initialize: function () {
                ...
                $(document).on('click', '.funiter-mapper-bot > .edit-image > .image-wrap > img', this.create);
        ...
        $(document).on('init_funiter_mapper', function (event) {
            if (!$(document).data('funiter_mapper_settings_initialized')) {
                ...
                $('#general-settings').find('input, select, textarea').each(function () {
                    if (window.funiter_mapper_settings[$(this).attr('name')]) {
                        $(this).val(window.funiter_mapper_settings[$(this).attr('name')]);

                        if ($(this).attr('type') == 'hidden' && $(this).next().attr('type') == 'checkbox') {
                            if (parseInt(window.funiter_mapper_settings[$(this).attr('name')])) {
                                $(this).next().attr('checked', 'checked');
                            } else {
                                $(this).next().removeAttr('checked');
                            }
                    ...
                    $(this).attr('name', 'funiter_mapper_settings[' + $(this).attr('name') + ']');
                ...
                $('#general-settings').on('change', 'input[type="radio"], select', function () {
                    var option = $(this).attr('name').match(/funiter_mapper_settings\[([^\]]+)\]/), value = $(this).val();
                ...
                $(document).click(function (event) {
                    ...
                    $('.funiter-mapper .wp-picker-holder .iris-picker').each(function () {
                    ...
                    $('#general-settings.editing, .funiter-pin.editing').each(function () {
                ...
                // Track data changes.
                var data_fields = '.funiter-mapper input, .funiter-mapper select, .funiter-mapper textarea';

                $(document).on('change', data_fields, function () {
                    if ($(this).attr('name') && $(this).attr('name').indexOf('funiter_mapper_') > -1) {
                        $('#post #publish').attr('data-changed', 'yes');
                    }
                });
                ...
                $(window).on('beforeunload', function () {
                    ...
                    if (submit_btn.length && submit_btn.attr('data-changed') == 'yes') {
                        return funiter_mapper.text.ask_for_saving_changes;
                    }
                });

                $(document).data('funiter_mapper_settings_initialized', true);
            }

            if (!window.funiter_mapper_pins_app && $('.funiter-mapper-bot > .edit-image > .image-wrap').length) {
                // Init pin list view.
                window.funiter_mapper_pins_app = new PinListView;
            }
        });
    };
