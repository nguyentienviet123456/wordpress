<?php
if ( ! class_exists( 'Funiter_Live_Search' ) ) {
	class Funiter_Live_Search {
		public $key     = '_funiter_live_search_settings';
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_funiter_live_search', array( $this, 'get_results' ) );
			add_action( 'wp_ajax_nopriv_funiter_live_search', array( $this, 'get_results' ) );
			add_shortcode( "funiter_live_search_form", array( $this, 'live_search_form' ) );
		}
		...
		public function scripts() {
			...
			if ( $enable_live_search != false ) {
				...
				wp_enqueue_script( 'funiter-live-search', trailingslashit( plugin_dir_url( __FILE__ ) ) . 'live-search.js', array( 'jquery' ), '1.0' );
				wp_localize_script( 'funiter-live-search', 'funiter_ajax_live_search', array(
					                                         'ajaxurl'                            => admin_url( 'admin-ajax.php' ),
					                                         'security'                           => wp_create_nonce( 'funiter_ajax_live_search' ),
					                                         'view_all_text'                      => esc_html__( 'View All', 'funiter-toolkit' ),
					                                         'product_matches_text'               => esc_html__( 'Product Matches', 'funiter-toolkit' ),
					                                         'results_text'                       => esc_html__( 'Results', 'funiter-toolkit' ),
					                                         'funiter_live_search_min_characters' => $min_chars,
					                                         'limit_char_message'                 => sprintf( esc_html__( 'Please enter at least %s characters to search', 'funiter-toolkit' ), $min_chars )
				                                         )
				);
			}
		}
		...
		public function get_results() {
			...
			$GLOBALS['funiter_live_search_settings'] = $data;
			...
			if ( $products ) {
				foreach ( $products as $key => $product ) {
					...
					if ( $data['show_suggestion'] == 'on' && in_array( 'sku', $data['search_in'] ) ) {
						$products[ $key ]->funiter_sku = $product->get_sku();
					}
					if ( $product ) {
				...
				if ( $data['show_suggestion'] == 'on' ) {
					foreach ( $products as $product ) {
						...
						if ( in_array( 'sku', $data['search_in'] ) && ! isset( $return_data['suggestion'] ) ) {
							// Convert HTML tag and shortcode to space.
							$content_search = preg_replace( '/\\[[^\\]]*\\]|<[^>]*>/', ' ', $product->funiter_sku );
							// Find keyword.
							$position_keyword = stripos( $content_search, $data['keyword'] );
							if ( $position_keyword !== false && $position_keyword + strlen( $data['keyword'] ) < strlen( $content_search ) ) {
								// Get suggestion of keyword in content.
			...
			wp_send_json( array( 'message' => esc_html__( 'No results.', 'funiter-toolkit' ) ) );
			wp_die();
		}
		...
		public static function posts_where( $where ) {
			global $wpdb, $funiter_live_search_settings;
			// Prepare search coverages.
			$columns = array();
			if ( in_array( 'title', $funiter_live_search_settings['search_in'] ) ) {
				$columns[] = ' ' . $wpdb->posts . '.post_title LIKE "%' . sanitize_text_field( $funiter_live_search_settings['keyword'] ) . '%" ';
			}
			if ( in_array( 'description', $funiter_live_search_settings['search_in'] ) ) {
				$columns[] = ' ' . $wpdb->posts . '.post_excerpt LIKE "%' . sanitize_text_field( $funiter_live_search_settings['keyword'] ) . '%" ';
			}
			if ( in_array( 'content', $funiter_live_search_settings['search_in'] ) ) {
				$columns[] = ' ' . $wpdb->posts . '.post_content LIKE "%' . sanitize_text_field( $funiter_live_search_settings['keyword'] ) . '%" ';
			}
			if ( in_array( 'sku', $funiter_live_search_settings['search_in'] ) ) {
				$columns[] = '( ' . $wpdb->postmeta . '.meta_key = "_sku" AND ' . $wpdb->postmeta . '.meta_value LIKE "%' . sanitize_text_field( $funiter_live_search_settings['keyword'] ) . '%" )';
			}
			if ( count( $columns ) ) {
		...
		public function live_search_form( $atts, $content = '' ) {
			$default = array(
				'placeholder' => __( 'Search products', 'funiter-toolkit' ),
			);
			$atts    = shortcode_atts( $default, $atts );
			ob_start();
			?>
            <form method="get" action="<?php echo esc_url( home_url( '/' ) ) ?>"
                  class="block-search funiter-live-search-form">
			...
			return apply_filters( 'funiter_output_live_search_form', $html, $atts );
		}
	}
	
	new Funiter_Live_Search();
}
