...
if ( !class_exists( 'Toolkit_MailChimp_Settings' ) ) {
	class Toolkit_MailChimp_Settings
	{
		...
		public function add_plugin_page()
		{
			...
			add_submenu_page( 'funiter_menu', __( 'MailChimp Settings', 'funiter-toolkit' ), __( 'MailChimp', 'funiter-toolkit' ), 'manage_options', 'mailchimp-settings', array( $this, 'create_admin_page' ) );
		}
		...
		public function create_admin_page()
		{
			?>
            <div class="wrap">
                <h2><?php _e( 'Mail Chimp Settings', 'funiter-toolkit' ); ?></h2>
		...
		public function page_init()
		{
			...
			add_settings_section(
				...
				__( 'Settings', 'funiter-toolkit' ), // Title
				array( $this, 'print_section_info' ), // Callback
				'mailchimp-settings' // Page
			);
			add_settings_field(
				'api_key', // ID
				__( 'Mail Chimp API Key', 'funiter-toolkit' ), // Title
			...
			if ( isset ( $api_key ) && !empty ( $api_key ) ) {
				add_settings_field(
					'email_lists', // ID
					__( 'Email Lists', 'funiter-toolkit' ), // Title
				...
				add_settings_field(
					'success_message', // ID
					__( 'Success message', 'funiter-toolkit' ), // Title
		...
		public function api_key_callback()
		{
			...
			printf(
				...
				__( 'Enter your mail Chimp API key to enable a newsletter signup option with the registration form.', 'funiter-toolkit' )
			);
			printf( wp_kses( __( '<a href="%s" target="_blank">Click here to get your Mailchimp API key</a>', 'funiter-toolkit' ), array( 'a' => array( 'href' => array(), 'target' => array() ) ) ), 'https://admin.mailchimp.com/account/api' );
		}
		...
		public function email_lists_callback()
		{
			...
			if ( isset ( $api_key ) && !empty ( $api_key ) ) {
				...
				if (!empty($lists)) {
					...
					echo '<strong>' . esc_html__('Error connecting to Mailchimp', 'funiter-toolkit') . '</strong>';
				}
			}
		}
