	...
	class Toolkit_Mailchimp
	{
		...
		function admin_notice()
		{
            ...
            <div class="updated">
                <p><?php
					printf(
						__( 'Please enter Mail Chimp API Key in <a href="%s" target="_blank">here</a>', 'funiter-toolkit' ),
		...
		public function submit_mailchimp_via_ajax()
		{
			...
			$api_key         = "";
			$list_id         = "";
			$success_message = esc_html__( 'Your email added...', 'funiter-toolkit' );
			if ( $this->options ) {
			...
			$response['message'] = esc_html__( 'Failed', 'funiter-toolkit' );
			$merge_vars          = array();
			if ( class_exists( 'MCAPI' ) ) {
