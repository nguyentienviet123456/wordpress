...
if ( !class_exists( 'MCAPI' ) ) {
	class MCAPI
	{
		...
		private function show_error( $message )
		{
			...
			add_settings_error( 'funiter-api', 'funiter-api-error', $message, 'error' );
		...
		{
			$message .= '<br /><br />' . sprintf( '<a href="%s">' . __( 'Read more about common connectivity issues.', 'funiter-toolkit' ) . '</a>', 'https://kb.mc4wp.com/solving-connectivity-issues/#utm_source=wp-plugin&utm_medium=mailchimp-for-wp&utm_campaign=settings-notice' );
		...
		public function call( $method, array $data = array() )
		{
			...
			$request_args   = array(
				...
				'sslverify' => apply_filters( 'funiter_use_sslverify', true ),
			);
		...
		private function get_headers()
		{
			...
			$headers = array(
				...
				'User-Agent' => 'FUNITER/' . FUNITER_TOOLKIT_VERSION . '; WordPress/' . $wp_version . '; ' . get_bloginfo( 'url' ),
			);
