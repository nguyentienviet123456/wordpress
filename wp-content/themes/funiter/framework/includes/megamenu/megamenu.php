<?php
if ( !class_exists( 'Funiter_MegaMenu' ) ) {
	class Funiter_MegaMenu
	{
		...
		function register_scripts( $hook )
		{
			...
			wp_localize_script( 'funiter-megamenu', 'funiter_ajax_backend', array(
					'ajaxurl'  => admin_url( 'admin-ajax.php' ),
					'security' => wp_create_nonce( 'funiter_ajax_backend' ),
				)
			);
		}
		...
		public function set_fonts()
		{
			$this->lits_fonts = array(
				array(
					'title' => esc_html__( 'Font Awesome', 'funiter' ),
					'fonts' => array(
            ...
            <div class="icons-popup">
                <div class="inner">
					<?php if ( count( $this->lits_fonts ) > 0 ): ?>
                        <div class="funiter-modal-header funiter-text-center">
                            <label><?php echo esc_html__( 'Search Icon: ', 'funiter' ); ?></label>
                            <input type="text"
                                   placeholder="<?php echo esc_attr__( 'Search a Icon...', 'funiter' ); ?>"
                                   class="funiter-icon-search"/>
                        </div>
						...
						<?php endforeach; ?>
					<?php else: ?>
						<?php esc_html_e( 'No Font', 'funiter' ); ?>
					<?php endif; ?>
                </div>
            </div>
			<?php
		...
		public function addShortcodesCustomCss_megamenu()
		{
			...
			if ( $posts_array ) {
				...
				foreach ( $posts_array as $post ) {
					...
					$shortcodes_custom_css .= get_post_meta( $post->ID, '_Funiter_Shortcode_custom_css', true );
				}
				if ( !empty( $shortcodes_custom_css ) ) {
					wp_add_inline_style( 'funiter-style', $shortcodes_custom_css );
				}
			}
		}
	}

	new Funiter_MegaMenu();
}