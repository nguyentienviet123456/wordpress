 ...
 * @since    1.0
 */
if ( ! class_exists( 'Funiter_framework' ) ) {
	class Funiter_framework {
		/**
		...
		public function __construct() {
			...
			add_filter( 'funiter_carousel_data_attributes', array( $this, 'funiter_carousel_data_attributes' ), 10, 2 );
			add_filter( 'funiter_getProducts', array( $this, 'funiter_getProducts' ), 10, 3 );
			/* CUSTOM IMAGE ELEMENT */
			if ( ! is_admin() ) {
				add_filter( 'post_thumbnail_html', array( $this, 'funiter_post_thumbnail_html' ), 10, 5 );
				add_filter( 'vc_wpb_getimagesize', array( $this, 'funiter_vc_wpb_getimagesize' ), 10, 3 );
				add_filter( 'wp_kses_allowed_html', array( $this, 'funiter_wp_kses_allowed_html' ), 10, 2 );
				add_filter( 'wp_get_attachment_image_attributes', array(
					$this,
					'funiter_lazy_attachment_image'
				), 10, 3 );
			}
			add_filter( 'funiter_resize_image', array( $this, 'funiter_resize_image' ), 10, 5 );
			$this->includes();
		}
		...
		function body_class( $classes ) {
			...
			if ( class_exists( 'WooCommerce' ) ) {
				if ( is_cart() ) {
					$cat_layout = Funiter_Functions::funiter_get_option( 'cart_layout', 'full_width' );
					$classes[]  = 'cat-layout-' . esc_attr( $cat_layout );
				}
				if ( Funiter_Functions::is_mobile() ) {
					...
					if ( $enable_single_product_mobile && is_product() ) {
		...
		public function enqueue_scripts( $hook ) {
			...
			wp_enqueue_style( 'funiter-custom-admin', get_theme_file_uri( '/framework/assets/admin.css' ), array(), '1.0' );
		}
		...
		function funiter_carousel_data_attributes( $prefix = '', $atts ) {
			$responsive = array();
			$slick      = array();
			$results    = '';
			...
			if ( isset( $atts[ $prefix . 'responsive_margin' ] ) && $atts[ $prefix . 'responsive_margin' ] >= 480 ) {
				$responsive[0]['settings']['slidesMargin'] = 14;
			}
			if ( isset( $atts[ $prefix . 'responsive_margin' ] ) && $atts[ $prefix . 'responsive_margin' ] >= 768 ) {
				$responsive[1]['settings']['slidesMargin'] = 20;
			}
			...
			if ( isset( $atts[ $prefix . 'responsive_margin' ] ) && $atts[ $prefix . 'responsive_margin' ] >= 1200 ) {
				$responsive[3]['settings']['slidesMargin'] = 30;
			}
			...
			$results = apply_filters( 'funiter_filter_carousel_data_attributes', $results, $prefix, $atts );
			
			return htmlspecialchars( $results );
		}
		
		function funiter_getProducts( $atts, $args = array(), $ignore_sticky_posts = 1 ) {
			extract( $atts );
			...
			switch ( $target ):
				...
				case 'featured_products' :
					...
					break;
				case 'on_new' :
					$newness            = Funiter_Functions::funiter_get_option( 'funiter_product_newness', 7 );    // Newness in days as defined by option
		...
		function funiter_lazy_attachment_image( $attr, $attachment, $size ) {
			$enable_lazy = Funiter_Functions::funiter_get_option( 'funiter_theme_lazy_load' );
			if ( $enable_lazy == 1 ) {
		...
		function funiter_wp_kses_allowed_html( $allowedposttags, $context ) {
		...
		function funiter_post_thumbnail_html( $html, $post_ID, $post_thumbnail_id, $size, $attr ) {
			$enable_lazy = Funiter_Functions::funiter_get_option( 'funiter_theme_lazy_load' );
			if ( $enable_lazy == 1 ) {
		...
		function funiter_vc_wpb_getimagesize( $img, $attach_id, $params ) {
			$enable_lazy = Funiter_Functions::funiter_get_option( 'funiter_theme_lazy_load' );
			if ( $enable_lazy == 1 ) {
		...
		function funiter_get_attachment_image( $attachment_id, $src, $width, $height, $lazy ) {
			$html     = '';
			...
			if ( $src ) {
				 ...
				 */
				$attr = apply_filters( 'funiter_get_attachment_image_attributes', $attr, $attachment );
		...
		function funiter_resize_image( $attachment_id = null, $width, $height, $crop = false, $use_lazy = false ) {
			$original    = false;
			$image_src   = array();
			$enable_lazy = Funiter_Functions::funiter_get_option( 'funiter_theme_lazy_load' );
			...
			if ( ! empty( $actual_file_path ) && file_exists( $actual_file_path ) ) {
				if ( $original == false && ( $image_src[1] > $width || $image_src[2] > $height ) ) {
					...
					if ( file_exists( $cropped_img_path ) ) {
						...
						$vt_image        = array(
							...
							'width'  => $width,
							'height' => $height,
							'img'    => $this->funiter_get_attachment_image( $attachment_id, $cropped_img_url, $width, $height, $use_lazy ),
						);
					...
					if ( $crop == false ) {
						...
						if ( file_exists( $resized_img_path ) ) {
							...
							$vt_image        = array(
								...
								'img'    => $this->funiter_get_attachment_image( $attachment_id, $resized_img_url, $proportional_size[0], $proportional_size[1], $use_lazy ),
							);
					...
					$vt_image     = array(
						...
						'img'    => $this->funiter_get_attachment_image( $attachment_id, $new_img, $new_img_size[0], $new_img_size[1], $use_lazy ),
					);
				...
				$vt_image = array(
					...
					'width'  => $image_src[1],
					'height' => $image_src[2],
					'img'    => $this->funiter_get_attachment_image( $attachment_id, $image_src[0], $image_src[1], $image_src[2], $use_lazy ),
				);
				...
				$vt_image        = array(
					...
					'width'  => $width,
					'height' => $height,
					'img'    => ( $original == false ) ? $this->funiter_get_attachment_image( $attachment_id, $url_placeholder, $width, $height, false ) : '',
				);
